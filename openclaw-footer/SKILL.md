...
- Discord and Telegram share the same agent-runner runtime patch — no separate runtime modification is needed.
- `discord footer check` → verify the shared patch is present.
- `discord footer apply` → apply via the shared Telegram patch script.
- Discord footer format is identical to Telegram: `────` separator + rich footer line.
