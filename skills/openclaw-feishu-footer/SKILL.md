...
5. The canonical footer fields are now `Model | CWD | Thinking | Context | Tokens | Usage`.
...
`Model | CWD | Thinking | Context | Tokens | Usage`

Session/date and time are intentionally omitted. The usage summary (`Usage: ...`) is fetched from the same shared provider-usage module used by Telegram and Discord and formatted by the shared module, e.g. `Usage: 74%/4h, Week 80%/5d 8h`.
